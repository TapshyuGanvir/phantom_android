...
public class UploadTrail {
    ...
    public UploadTrail(Context context, UploadInterface uploadInterface, CMGraph cmGraph) {
        this.context = context;
        this.uploadInterface = uploadInterface;
        this.entityName = AppConstants.TRAIL_TABLE_NAME;
        this.cmGraph = cmGraph;
        this.hasAttachments = false;
        this.uploadTraversalGraph = false;
...
//            updateMetaDataEntryIfFailed();
    ...
    private void uploadEditedTrail() {
        try{
            JSONObject payloadJsonAdd = createPayloadJson(context,AppConstants.EDIT);
            Log.d("payloadjson", "uploadAddedTrail: "+payloadJsonAdd);
            if(payloadJsonAdd.has("status")){
                if(payloadJsonAdd.getString("status").equalsIgnoreCase("success")){
                    if (!payloadJsonAdd.has("dataJson")) {
//                        updateMetaDataEntryIfFailed();
                        uploadInterface.OnUploadFinished(false, payloadJsonAdd);
                    } else {
                        JSONObject dataJson = payloadJsonAdd.getJSONObject("dataJson");
...
//                updateMetaDataEntryIfFailed();
    ...
    private void uploadAddedTrail() {
        try{
            JSONObject payloadJsonAdd = createPayloadJson(context,AppConstants.ADD);
            Log.d("payloadjson", "uploadAddedTrail: "+payloadJsonAdd);
            if(payloadJsonAdd.has("status")){
                if(payloadJsonAdd.getString("status").equalsIgnoreCase("success")){
                    if (!payloadJsonAdd.has("dataJson")) {
...
//                updateMetaDataEntryIfFailed();
    ...
    private JSONObject createPayloadJson(Context context, String operation) {
...
//                Map<String, HashMap<Object, String>> addedIdMap=new LinkedHashMap<>();
                Map<String, HashMap<Object, String>> addedIdMap = null;
                try {
                    if(operation.equals(AppConstants.ADD)){
                        addedIdMap= EditMetaDataTable.getAddedId(context,AppConstants.TRAIL_TABLE_NAME);
                    }else if(operation.equals(AppConstants.EDIT)){
                        addedIdMap= EditMetaDataTable.getEditedId(context,AppConstants.TRAIL_TABLE_NAME);
                    }
...
//                    addedIdMap.put(entityName, idList);
                ...
                if (cmGraph != null && addedIdMap != null && !addedIdMap.isEmpty()) {
                    JSONObject payload = createFeatureJson(surveyName, addedIdMap, cmGraph, operation);//create edit json
                    if (payload != null) {
    ...
    private void uploadFeatureToServerData(String operationName, JSONObject dataJson) {
        ...
        try {
            ...
            StringRequest stringRequest = new StringRequest(Request.Method.POST, uploadUrl, response -> {
                ...
                handleFeatureEditSuccessResponse(operationName);
            },
...
//            updateMetaDataEntryIfFailed();
    ...
    public void handleFeatureEditSuccessResponse(String operationName) {
        String taskName = "handleFeatureEditSuccessResponse";
        ReveloLogger.info(className, taskName, "Feature edit succeeded!!");
        boolean isDelete=EditMetaDataTable.deleteOperation(context,operationName,entityName);
        Log.d("isdelete", "handleFeatureEditSuccessResponse: "+String.valueOf(isDelete));
